/**
 * Education options and degree-aware eligibility (mirrors backend logic).
 * Used for profile dropdown and client-side eligibility display.
 */
...
function getRank(userEducation) {
  const norm = normalize(userEducation)
  if (!norm) return null
...
export function isEducationEligible(userEducation, requiredEducation) {
  ...
  if (userRank === null) {
    const reqNorm = normalize(requiredEducation)
...
export function isEducationEligibleForKeys(userEducation, requiredKeys) {
  if (!userEducation || !String(userEducation).trim()) return false
  ...
  const keySet = new Set(requiredKeys.map((k) => normalize(k)))
...
/** Get display label for a stored education value (e.g. "b.tech" -> "B.Tech / B.E") */
export function getEducationLabel(value) {
  if (!value) return ''
  const v = normalize(value)
  const opt = EDUCATION_OPTIONS.find((o) => o.value === v)
